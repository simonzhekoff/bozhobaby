<?php namespace Buzz\Oksireserv\Facades;
...
use Buzz\Oksireserv\Classes\DatesResolver;
use Buzz\Oksireserv\Classes\Variables;
use Buzz\Oksireserv\Mailers\ReservationAdminMailer;
 ...
 *
 * @package Buzz\oksireserv\Facades
 */
class ReservationsFacade
{
    ...
    public function storeReservation($data)
    {
        ...
        if (!empty($_POST["address"])) {
            // If the honeypot field is not empty, it's likely a bot, so exit the script without sending an email
            exit;
        }
        ...
        // place for extending
        Event::fire('buzz.oksireserv.processReservation', [&$data]);
    ...
    public function transformDateTime($data)
    {
        // validate date
        if (empty($data['date'])) {
            throw new ApplicationException(Lang::get('buzz.oksireserv::lang.errors.empty_date'));
        }

        // validate time
        if (empty($data['time'])) {
            throw new ApplicationException(Lang::get('buzz.oksireserv::lang.errors.empty_hour'));
        }
        ...
        if ($dateTime->timestamp < Carbon::now()->timestamp) {
            throw new ApplicationException(Lang::get('buzz.oksireserv::lang.errors.past_date'));
        }

        // validate days off
        if (!in_array($dateTime->dayOfWeek, $this->getWorkingDays())) {
            throw new ApplicationException(Lang::get('buzz.oksireserv::lang.errors.days_off'));
        }
        ...
        if ($timeToMinute < $workTimeFrom || $timeToMinute > $workTimeTo) {
            throw new ApplicationException(Lang::get('buzz.oksireserv::lang.errors.out_of_hours'));
        }
    ...
    private function checkLimits()
    {
        if ($this->isCreatedWhileAgo()) {
            throw new ApplicationException(Lang::get('buzz.oksireserv::lang.errors.please_wait'));
        }
    }
    ...
    {
        // protection time
        $time = Config::get('buzz.oksireserv::config.protection_time', '-30 seconds');
