<?php namespace Buzz\Oksireserv\Components;
...
{
    const PATH_PICKADATE_COMPRESSED = '/plugins/buzz/oksireserv/assets/vendor/pickadate/lib/compressed/';
	...
	{
		return [
			'name' => 'buzz.oksireserv::lang.reservationform.name',
			'description' => 'buzz.oksireserv::lang.reservationform.description',
		];
	}
    ...
    public function onSubmit()
    {
        ...
        if (Session::token() !== Input::get('_token')) {
            throw new ApplicationException(Lang::get('buzz.oksireserv::lang.errors.session_expired'));
        }
	...
	public function onRun()
	{
		...
		if (Input::get($this->alias . '-submit')) {
            ...
            // check CSRF token
            if (Session::token() !== Input::get('_token')) {
                $error = Lang::get('buzz.oksireserv::lang.errors.session_expired');

            } else {
                try {
                    $data = Input::all();
                    $facade->storeReservation($data);
                    $msg = Lang::get('buzz.oksireserv::lang.reservationform.success');
                    Flash::success($msg);
                    ...
                    $error = Lang::get('buzz.oksireserv::lang.errors.exception');
                }
            }
		}
    ...
    protected function injectAssets()
    {
        ...
        $this->addJs('/plugins/buzz/oksireserv/assets/js/convert.js');
        $this->addJs('/plugins/buzz/oksireserv/assets/js/reservationform.js');
    }
