...
class Reservation extends Model
{
    ...
    public $rules = [
        ...
        'usluga' => 'required|max:300',
        'message' => 'required|max:3000',
    ];

    public $customMessages = [
        'reservation' => 'buzz.oksireserv::lang.errors.already_booked',
    ];

    public $fillable = [
        'status', 'date', 'locale', 'email', 'name', 'lastname',
        'street', 'town', 'zip', 'phone', 'message', 'usluga',
    ];
    ...
    {
        $statusIdent = Config::get('buzz.oksireserv::config.statuses.received', 'received');
    ...
    {
        $length = Config::get('buzz.oksireserv::config.hash', 32);
        if ($length == 0) {
            return null;
        }
    ...
    {
        // init
        $min = Config::get('buzz.oksireserv::config.number.min', 123456);
        $max = Config::get('buzz.oksireserv::config.number.max', 999999);
        if ($min == 0 || $max == 0) {
            return null;
        }
